...
const EpicForm = (props) => {
  ...
  useEffect(() => {
    ...
    apiGetAllParentAccounts()
      .then((response) => {
        ...
        response.forEach((parentsaccount, index) => {
          parentsaccounts.push({
            value: parentsaccount.id,
            label: parentsaccount.name,
          });
        });
        setParentAccounts(parentsaccounts);
      })
      .catch((err) => {
  ...
  const onChangeParentAccount = (e) => {
    const vPA = e.value;
    setParentAccountId(vPA);
  };
