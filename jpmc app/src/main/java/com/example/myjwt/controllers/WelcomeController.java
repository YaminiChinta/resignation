...
public class WelcomeController {
	...
	public List<UsersDetail> getAllUser() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			System.out.println("----------------------------------------:"+list.size()+":::::"+user.getAssociateId());
	...
	private void createSettings() {
		...
		String[][] settings = {
				...
				{ "ADM", "CIB Lead", "190741", "text" }, { "ADM", "AM Lead", "121870", "text" },
				{ "ADM", "CCB Lead", "106716", "text" }, { "ADM", "CB Lead", "122539", "text" },
				{ "ADM", "CT Lead", "106716", "text" }, { "ADM", "GTI Lead", "106716", "text" },
				{ "ADM", "Cyber Lead", "106716", "text" }, { "ADM", "EDL", "254395", "text" },
				{ "ADM", "PDL", "231612", "text" }, { null, "Cognizant Email Working?", "false", "boolean" },
				{ null, "Default Email", "narenkgcts@outlook.com", "text" },
	...
	public void createDefaultSBUs() {
		List<SBU> sbuList = new ArrayList<>();
		sbuList.add(new SBU(null, "BFS", 254395L, "Admin1"));
		sbuRepository.saveAll(sbuList);
	}

	public void createDefaultPDLs() {
		List<PDL> pdlList = new ArrayList<>();
		List<SBU> all = sbuRepository.findAll();
		pdlList.add(new PDL(231612L, "PDL1", all.get(0)));
		pdlRepository.saveAll(pdlList);
	}

	public void createDefaultParentAccounts() {
		List<ParentAccount> parentAccountList = new ArrayList<>();
		List<PDL> pdlList = pdlRepository.findAll();
		parentAccountList.add(new ParentAccount(null, "JPMC", 254395L, "EDL1", pdlList.get(0)));
		parentAccountRepository.saveAll(parentAccountList);
	}
...
//    }
	...
	public void createDefaultStoryStatus() {
		List<String> storyStatus = new ArrayList<>(
				Arrays.asList("Accepted", "In Progress", "Cancelled", "Not Started"));
		List<Category> categoryList = new ArrayList<>();
		for (String status : storyStatus) {
			Category temp = new Category();
			temp.setGroupKey(AppConstants.STORY_STATUS);
			temp.setCatGroup(AppConstants.STORY_STATUS);
	...
	public void createDefaultStoryPriority() {
		List<String> storyPriority = new ArrayList<>(Arrays.asList("High", "Mid", "Low"));
		List<Category> categoryList = new ArrayList<>();
	...
	public void setInitialValuesInDB() {
		...
		if (user == null) {
			...
			createHolidays();
			 createDefaultSBUs();
			 createDefaultPDLs();
			 createDefaultParentAccounts();
//            createDefaultEpics();
			 createDefaultSprints();
			createDefaultStoryStatus();
			createDefaultStoryPriority();
		}
	}
