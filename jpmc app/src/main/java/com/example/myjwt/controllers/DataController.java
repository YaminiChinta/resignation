...
//		}
    ...
    private void sendEmailToTag(Category category, Referrals referrals) throws MessagingException, TemplateException, IOException {
    ...
    public ResponseEntity<?> getAssignmentUserIdsFromServiceLine(@PathVariable String serviceLine) {
        ...
        try {
            AssignmentReport assignmentReport = assignmentReportRepository.findFirstByOrderByIdDesc().orElseThrow(() -> new Exception("No reports found"));
            List<HashMap> list = assignmentUserRepository.findByAssignmentReportAndServiceLine(assignmentReport, serviceLine).stream().map(associate -> new HashMap(Map.of("label", associate.getAssociateName(), "value", associate.getAssociateID()))).collect(Collectors.toList());
    ...
    public ResponseEntity<?> updateSettingsData(@Valid @RequestBody ArrayList<KeyValue> changedData,
        ...
        try {
            ...
            return ResponseEntity.created(location)
                    .body(new ApiResponse(true, "Settings updated successfully!!"));
