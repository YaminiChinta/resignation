...
public class LeavesTrackerService {
    ...
    private void sendConflictMail(ArrayList<Leave> temp) throws Exception {
        ...
        for (AssociateGroup g : groups) {
            ...
            if (flag) {
                ...
                sendEmailToManager(leaveListMap, g);
            }
        }
    }
    ...
    private void sendEmailToManager(Map<Leave, List<AssignmentUser>> leaveListMap, AssociateGroup group) throws MessagingException, TemplateException, IOException {
...
//            System.out.println("entered users");
            Optional<User> user = userRepository.findByAssociateId(i.getAssociateID());
            if (user.isPresent()) {
                users.add(user.get());
            }
    ...
    public ArrayList<LeaveResponse> getAllUserLeavesByServiceLine(int year, String serviceLine)
        ...
        for (AssignmentUser i : assignmentUsers) {
         ...
         if( i.getBillabilityStatus()!=true)
          	 continue;
